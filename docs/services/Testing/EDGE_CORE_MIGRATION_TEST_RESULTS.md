...
**Migration:** `services/` → `edge_services/` + `core_services/`
...
### Dockerfiles
- ✅ `/core_services/storage/Dockerfile` - Updated paths
- ✅ `/core_services/api/Dockerfile` - Updated paths
...
### Requirements
- ✅ `/core_services/storage/requirements.txt` - Added prometheus-client
...
**Solution:** Renamed directories to use underscores (`edge_services`, `core_services`) and updated `pilot.py` to add directories to `sys.path`:
```python
...
The migration from `/services` to `/edge_services` and `/core_services` is **complete and fully operational**.
